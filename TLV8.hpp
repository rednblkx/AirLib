...
public:
    ...
    iterator add(uint8_t tag, size_t len, const uint8_t* val) {
        items.emplace_back(tag, val, len);
        return --items.end(); // Return iterator to the added element
    }

    iterator add(uint8_t tag, std::vector<uint8_t> value) {
        items.emplace_back(tag, std::move(value));
        return --items.end();
    }

    // Convenience overloads for adding
    iterator add(uint8_t tag, uint8_t val) {
        uint8_t v[] = {val};
        return add(tag, 1, v);
    }
    iterator add(uint8_t tag) { return add(tag, 0, nullptr); }
    ...
    size_t get(uint8_t* buffer, size_t buffer_size) const {
        ...
        if (buffer_size < required_size) {
            throw std::runtime_error(
                "Buffer too small for packing. Required: " +
                std::to_string(required_size) +
                ", Available: " + std::to_string(buffer_size)
            );
        }
    ...
    pack_overflow:
         ...
         throw std::runtime_error("Internal packing error: Buffer overflow detected.");
    }
    ...
    void parse(const uint8_t* buffer, size_t buffer_size) {
        ...
        while (current_pos < end_pos) {
            ...
            if (current_pos + 1 > end_pos) {
                throw std::runtime_error("Unpack error: Insufficient data for tag");
            }
            ...
            if (current_pos + 1 > end_pos) {
                throw std::runtime_error(
                    "Unpack error: Insufficient data for length byte (tag: 0x" +
                    to_hex(tag) + ")"
                );
            }
            size_t len = 0;
            ...
            if (use_ber_length_encoding && (len_byte & 0x80)) {
                ...
                if (len_bytes_count == 0 || len_bytes_count > sizeof(size_t)) {
                    throw std::runtime_error(
                        ...
                        to_hex(tag) + ", len_byte: 0x" + to_hex(len_byte) + ")"
                    );
                }
                if (current_pos + len_bytes_count > end_pos) {
                    throw std::runtime_error(
                        "Unpack error: Insufficient data for BER long length value (tag: 0x" +
                        to_hex(tag) + ")"
                    );
                }
                len = 0;
            ...
            if (current_pos + len > end_pos) {
                throw std::runtime_error(
                    "Unpack error: Insufficient data for value (tag: 0x" +
                    to_hex(tag) + ", length: " + std::to_string(len) + ")"
                );
            }
